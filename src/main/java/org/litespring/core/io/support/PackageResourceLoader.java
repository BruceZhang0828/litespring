...
public class PackageResourceLoader {
    ...
    public PackageResourceLoader(ClassLoader classLoader) {
        Assert.notNull("ResourceLoader must not be null",classLoader);
    ...
    public Resource[] getResources(String basePackage) throws IOException {
        ...
        URL url = cl.getResource(location);//获取全路径
        File rootDir = new File(url.getFile());//获取文件夹

        Set<File> matchingFiles = retrieveMatchingFiles(rootDir);//获取文件夹内的文件(set 去重)
        Resource[] result = new Resource[matchingFiles.size()];
        int i=0;
        for (File file : matchingFiles) {
            result[i++]=new FileSystemResource(file);//将文件 构造成 FileSystemResource数组
        }
        return result;
